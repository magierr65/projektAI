...
from src.preprocessing.feature_engineering import load_and_preprocess_rule_aided_data
from src.visualization.visualize import plot_hourly_mape, plot_hourly_predictions_comparison

def run_rule_aided_training(df):
    model_name = 'rule_aided'
    NeuronsNo = 50
    OutputNo = 1
    EpochsNo = 100
    K = 5
    ...
    for hour in ordered_hours:
        ...
        if len(X_hour) > 0:
            X_hour[0][Li1], X_hour[0][Li2], X_hour[0][Li3] = L_i_1, L_i_2, L_i_3
        ...
        if y_test.size > 0:
            for _ in range(K):
                ...
                hour_predictions.append(y_pred.flatten())
            
            avg_pred = np.mean(hour_predictions, axis=0)
            Pi_vector.append(avg_pred[0])
            mape = mean_absolute_percentage_error(y_test, avg_pred) * 100
            mape_vector.append(f"{mape:.2f}")
            print(f'HOUR: {hour} --- MAPE: {mape:.2f}%')
            L_i_3, L_i_2, L_i_1 = L_i_2, L_i_1, avg_pred[0]
        else:
            Pi_vector.append(0)
            mape_vector.append("0.00")
            print(f'HOUR: {hour} --- No test data')

    print(f"\n{NeuronsNo} neurons, {K} modules, Epochs: {EpochsNo}\n")
    avg_mape = np.mean([float(m) for m in mape_vector])
    print(f"Average MAPE: {avg_mape:.2f}%\n")

    plot_hourly_mape(mape_vector, model_name, title='Rule-Aided: Hourly MAPE')
    plot_hourly_predictions_comparison(y_true_24h, Pi_vector, model_name, title='Rule-Aided: Actual vs. Predicted Load')

if __name__ == '__main__':
    project_root = os.path.join(os.path.dirname(__file__), '..', '..')
    data_path = os.path.join(project_root, 'data', 'combined_data.csv')
    
    df_processed = load_and_preprocess_rule_aided_data(data_path)
    run_rule_aided_training(df_processed)
